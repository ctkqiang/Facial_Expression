package com.johnmelodyme.facialexpression.Helper;
...
public class MyHelper {
    ...
    public static File createTempFile(File file) {
        File directory = new File(Environment.getExternalStorageDirectory().getPath() + "/com.johnmelodyme.facialexpression");
        if (!directory.exists() || !directory.isDirectory()) {

            directory.mkdirs();
        }
        if (file == null) {
            file = new File(directory, "FaceExpression.jpg");
        }
        return file;
    }
    ...
    private static Bitmap compressPhoto(File photoFile, Bitmap bitmap) {
        try {
            ...
            bitmap.compress(Bitmap.CompressFormat.PNG, 70, fOutput);
            fOutput.close();
